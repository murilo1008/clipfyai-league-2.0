...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background: #000000; color: #ffffff;">
  <div style="max-width: 600px; margin: 0 auto; background: #000000;">
    ...
    <div style="padding: 0 24px 32px;">
      ...
      <!-- Discord - Presença Obrigatória + Call Amanhã -->
      <div style="background: linear-gradient(135deg, #5865F2 0%, #7289DA 100%); border-radius: 12px; padding: 24px; margin: 24px 0; text-align: center; border: 3px solid #FEE75C;">
        ...
        <!-- ALERTA: CALL AMANHÃ -->
        <div style="background: rgba(254, 231, 92, 0.2); border: 2px solid #FEE75C; border-radius: 8px; padding: 16px; margin: 20px 0;">
          <p style="margin: 0 0 8px 0; color: #FEE75C; font-size: 18px; font-weight: 900; line-height: 1.3;">
            🔴 AMANHÃ: PRIMEIRA CALL NO DISCORD!
          </p>
          <p style="margin: 0; color: #ffffff; font-size: 14px; font-weight: 600; line-height: 1.5;">
            Apresentação oficial da competição, regras e premiação
          </p>
        </div>
...
async function main() {
  try {
    ...
    // Buscar todos os usuários com role CLIPPER e status UNVERIFIED
    const unverifiedClippers = await prisma.user.findMany({
      where: {
        role: 'CLIPPER',
        clipperProfile: {
    ...
    // Sempre incluir o Murilo para acompanhamento
    emailList.push('murilo@clipfyai.com');
    
    // Adicionar emails dos clipadores não verificados (sem duplicatas)
    unverifiedClippers.forEach(user => {
      if (user.email && !emailList.includes(user.email)) {
        emailList.push(user.email);
      }
    });

    console.log(`📧 Total de emails a enviar: ${emailList.length}`);
    console.log(`📋 Lista: ${emailList.join(', ')}`);
    ...
    for (let i = 0; i < emailList.length; i += batchSize) {
      ...
      console.log(`\n📤 Enviando lote ${Math.floor(i / batchSize) + 1}/${Math.ceil(emailList.length / batchSize)}...`);
      console.log(`   Emails: ${batch.join(', ')}`);

      try {
        const { data, error } = await resend.emails.send({
          from: 'Clipfy League <onboarding@league.clipfyai.com>',
          to: batch,
        ...
        console.error(`❌ Erro ao enviar lote ${Math.floor(i / batchSize) + 1}:`, err);
        errorCount += batch.length;
      }

      // Aguardar 1 segundo entre lotes para evitar rate limiting
