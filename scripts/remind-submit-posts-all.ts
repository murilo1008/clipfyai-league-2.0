...
async function main() {
  try {
    ...
    // Filtrar clipadores com email válido
    const clippersWithEmail = applications.filter(
      (app) => app.clipperProfile.user?.email
    );

    console.log(`💡 Clipadores com email válido: ${clippersWithEmail.length}`);

    // Preparar lista de destinatários (todos os clipadores + murilo@clipfyai.com)
    const recipients = [
      ...clippersWithEmail.map((app) => ({
        email: app.clipperProfile.user!.email!,
        name: app.clipperProfile.artisticName || app.clipperProfile.fullName,
      })),
      {
    ...
    for (let i = 0; i < recipients.length; i++) {
      ...
      try {
        const result = await resend.emails.send({
          ...
          html: getSubmitPostsReminderEmailTemplate(recipient.name, campaign.name),
        });

        successCount++;
        console.log(`✅ [${i + 1}/${recipients.length}] Email enviado para: ${recipient.name} (${recipient.email})`);
        ...
        errorCount++;
        console.error(`❌ [${i + 1}/${recipients.length}] Erro ao enviar para ${recipient.name}:`, error.message);
      }
    }
