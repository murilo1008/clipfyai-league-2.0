...
import { CLIPFY_PRO_PRODUCT } from "@/lib/kiwify"

const resend = new Resend(process.env.RESEND_API_KEY)

// ============================================
// TEMPLATES DE EMAIL - CLIPFY PRO
// ============================================

/**
 * Template de email de boas-vindas para novo assinante PRO
 * Enviado quando um novo usuário se cadastra via assinatura
 */
export function getWelcomeProEmailTemplate(
  name: string,
  email: string,
  password: string
): string {
...
/**
 * Template de email de confirmação de pagamento para assinante existente
 * Enviado quando um usuário já cadastrado ativa a assinatura PRO
 */
...
/**
 * Template de email de cancelamento / encerramento de acesso PRO
 *
 * Este template cobre tanto cancelamento da assinatura legada quanto
 * reembolso/chargeback do pagamento único. O CTA leva o cliente para o
 * checkout vigente (PRO vitalício, R$ 197 — pagamento único).
 */
export function getSubscriptionCanceledEmailTemplate(
  name: string,
  email: string
): string {
  const proPrice = CLIPFY_PRO_PRODUCT.price.toLocaleString("pt-BR", {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  })
  const proInstallment = CLIPFY_PRO_PRODUCT.installmentPrice.toLocaleString("pt-BR", {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  })
  return `
          ...
          <div style="background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%); border-radius: 20px; padding: 40px; border: 1px solid rgba(255,255,255,0.1);">
            ...
            <div style="background: rgba(245,158,11,0.1); border: 1px solid rgba(245,158,11,0.3); border-radius: 12px; padding: 20px; margin-bottom: 30px;">
              <p style="margin: 0; font-size: 14px; color: #a0a0a0;">
                ...
                <strong style="color: #ffffff;">pagamento único de R$ ${proPrice}</strong>
                (em até ${CLIPFY_PRO_PRODUCT.maxInstallments}x de R$ ${proInstallment} no cartão) —
            ...
            <!-- CTA Button -->
            <div style="text-align: center;">
              <a href="${CLIPFY_PRO_PRODUCT.checkoutUrl}?email=${encodeURIComponent(email)}" style="display: inline-block; background: linear-gradient(90deg, #f59e0b, #ea580c); color: #000000; font-size: 16px; font-weight: bold; padding: 16px 40px; border-radius: 12px; text-decoration: none;">
...
/**
 * Envia email de boas-vindas para novo assinante PRO
 */
export async function sendWelcomeProEmail(params: {
  email: string
  name: string | null
  password: string
}): Promise<void> {
  ...
  console.log(`📧 Enviando email de boas-vindas PRO para: ${email}`)
  
  try {
    await resend.emails.send({
      from: "Clipfy League <noreply@league.clipfyai.com>",
      to: email,
      subject: "🎉 Bem-vindo ao Clipfy League PRO!",
      html: getWelcomeProEmailTemplate(name || "", email, password),
    })
...
/**
 * Envia email de confirmação de pagamento para assinante existente
 */
export async function sendPaymentConfirmationEmail(params: {
  ...
  console.log(`📧 Enviando email de confirmação de pagamento para: ${email}`)
  
  try {
    await resend.emails.send({
      from: "Clipfy League <noreply@league.clipfyai.com>",
      to: email,
      subject: "🚀 Seu acesso vitalício ao Clipfy League PRO foi ativado!",
      html: getPaymentConfirmationEmailTemplate(name || "", email),
    })
...
export async function sendSubscriptionCanceledEmail(params: {
  ...
  try {
    await resend.emails.send({
      from: "Clipfy League <noreply@league.clipfyai.com>",
      to: email,
      subject: "Seu acesso ao Clipfy League PRO foi encerrado",
      html: getSubscriptionCanceledEmailTemplate(name || "", email),
    })
