...
// ============================================
// TEMPLATES DE EMAIL - CLIPFY ULTRA
// ============================================

/**
 * Template de email de parabéns pela aquisição do ULTRA
 * Enviado quando um usuário (já existente) faz upgrade para ULTRA
 * NÃO cria senha - apenas parabeniza pela compra
 */
export function getUltraPurchaseConfirmationEmailTemplate(
  name: string,
  email: string
): string {
  return `
...
/**
 * Envia email de parabéns pela compra do ULTRA
 */
export async function sendUltraPurchaseConfirmationEmail(params: {
  email: string
  name: string | null
}): Promise<void> {
  const { email, name } = params
  
  console.log(`📧 Enviando email de parabéns ULTRA para: ${email}`)
  
  try {
    await resend.emails.send({
      from: "Clipfy League <noreply@league.clipfyai.com>",
      to: email,
      subject: "⚡ Parabéns! Você agora é Clipfy League ULTRA!",
      html: getUltraPurchaseConfirmationEmailTemplate(name || "", email),
    })
    console.log(`✅ Email de parabéns ULTRA enviado para: ${email}`)
  } catch (error) {
    console.error(`❌ Erro ao enviar email de parabéns ULTRA:`, error)
    throw error
  }
}
