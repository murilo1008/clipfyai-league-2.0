...
// ============================================
// TEMPLATES DE EMAIL - MANUAL DO CLIPADOR
// ============================================

/**
 * Template de email de parabéns pela aquisição do Manual do Clipador
 * Enviado quando um usuário compra o Manual (produto único)
 * NÃO cria senha - apenas parabeniza pela compra
 */
export function getManualPurchaseConfirmationEmailTemplate(
  name: string,
  email: string
): string {
  return `
...
/**
 * Envia email de parabéns pela compra do Manual do Clipador
 */
export async function sendManualPurchaseConfirmationEmail(params: {
  email: string
  name: string | null
}): Promise<void> {
  const { email, name } = params
  
  console.log(`📧 Enviando email de parabéns Manual do Clipador para: ${email}`)
  
  try {
    await resend.emails.send({
      from: "Clipfy League <noreply@league.clipfyai.com>",
      to: email,
      subject: "📚 Parabéns! Seu Manual do Clipador está liberado!",
      html: getManualPurchaseConfirmationEmailTemplate(name || "", email),
    })
    console.log(`✅ Email de parabéns Manual enviado para: ${email}`)
  } catch (error) {
    console.error(`❌ Erro ao enviar email de parabéns Manual:`, error)
    throw error
  }
}
