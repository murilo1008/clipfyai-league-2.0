...
const INITIAL_PRIZE_CONFIG: PrizeConfig = {
  ...
  dailyPrizeTable: { "1": 350, "2": 200, "3": 150, "4-15": 25 },
  bonusEnabled: true,
...
export function CreateCompetitionDialog({
  ...
  const createCampaign = api.admin.campaigns.create.useMutation({
    onMutate: () => {
      toast.loading("Criando competição...", { id: "create-campaign" })
    },
    onSuccess: () => {
      ...
      void utils.admin.getCampaignsStats.invalidate()
      void utils.campaign.getAll.invalidate()
      onOpenChange(false)
      resetForm()
    },
    onError: (error) => {
      toast.error(`Erro ao criar competição: ${error.message}`, {
        id: "create-campaign",
        duration: 6000,
      })
    },
  ...
  const resetForm = () => {
    ...
    setAffiliateLinksEnabled(false)
  ...
  const handleSubmit = () => {
    ...
    if (affiliateLinksEnabled) {
      for (const { key, platform } of AFFILIATE_PLATFORMS) {
        const link = affiliateLinks[key].trim()
        if (link && !isValidUrl(link)) {
          toast.error(
            `Link de afiliado inválido para ${platformConfig[platform].label}`,
            { description: "Use uma URL completa, ex.: https://exemplo.com" },
          )
          return
        }
      }
            ...
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <DialogTitle className="truncate text-lg font-bold tracking-tight sm:text-xl">
                  Nova <span className="text-gradient not-dark:brightness-[0.7] not-dark:saturate-[1.4]">Competição</span>
                </DialogTitle>
                <DialogDescription className="truncate text-xs sm:text-sm">
                  Configure sua competição de cortes e abra a arena
                </DialogDescription>
              </div>
            </div>
            <DialogClose asChild>
            ...
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="text-foreground inline-flex items-center gap-1.5 font-bold">
                {React.createElement(STEPS[step - 1]!.icon, {
                  className:
                    "size-3.5 text-brand-mint not-dark:text-primary",
                  weight: "fill",
                })}
                {STEPS[step - 1]!.label}
              </span>
    ...
    <>
      ...
      <div className="flex flex-col gap-2.5">
        <Label className="flex items-center gap-1.5 text-sm font-semibold">
          <ShieldCheck className="text-brand-mint not-dark:text-primary size-4" weight="fill" />
          Tipo de Competição
        </Label>
      ...
      <div className="flex flex-col gap-2.5">
        <Label className="flex items-center gap-1.5 text-sm font-semibold">
          <Crown className="size-4 text-violet-500 dark:text-violet-400" weight="fill" />
          Exclusividade PRO
        </Label>
        <label
          className={cn(
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between">
          <Label htmlFor="comp-desc" className="text-sm font-semibold">
            Descrição *
          </Label>
          <span
            className={cn(
  ...
  return (
    <>
      <SectionHero
        ...
        subtitle="Defina quando e onde a competição acontecerá"
      />
      ...
      {durationDays !== null && durationDays > 0 && (
        <div className="border-brand-cyan/25 bg-brand-cyan/5 not-dark:border-primary/25 not-dark:bg-primary/5 flex items-center gap-2.5 rounded-xl border px-3.5 py-2.5">
          <Clock className="text-brand-mint not-dark:text-primary size-4" weight="fill" />
  ...
  return (
    <>
      <SectionHero
        ...
        subtitle="Defina hashtags, menções e restrições — todas opcionais"
      />
    ...
    <>
      ...
      <div className="border-border/70 bg-card/40 flex flex-col gap-3.5 rounded-2xl border p-4 sm:p-5">
        ...
        <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 sm:gap-3">
          ...
          >
            ...
            <span className="min-w-0">
              ...
              <span className="text-muted-foreground mt-0.5 block text-xs leading-snug">
                Views multiplicado pela taxa de engajamento (likes, comentários,
                shares)
              </span>
            </span>
            {formData.rankingMetricType === "VIEWS_X_ENGAGEMENT" && (
        ...
        {formData.rankingMetricType === "VIEWS_X_ENGAGEMENT" && (
          <div className="animate-in fade-in-0 slide-in-from-top-1 flex items-start gap-2.5 rounded-xl border border-pink-500/25 bg-gradient-to-r from-purple-500/10 via-pink-500/10 to-orange-500/10 p-3 duration-200">
            <Info className="size-4 shrink-0 text-pink-500 dark:text-pink-400" weight="fill" />
            <p className="text-muted-foreground text-xs leading-relaxed">
              <strong className="text-foreground font-semibold">
                Fórmula:
              </strong>{" "}
              Score = Views × (1 + Taxa de Engajamento). Isso recompensa vídeos
              com maior interação da audiência!
            </p>
          </div>
        )}
      </div>
      ...
      >
        ...
        <div className="border-brand-cyan/25 bg-brand-cyan/5 not-dark:border-primary/25 not-dark:bg-primary/5 flex items-center gap-2 rounded-xl border px-3 py-2.5">
          <Info className="text-brand-mint not-dark:text-primary size-4 shrink-0" weight="fill" />
      ...
      <div className="rounded-2xl border border-amber-500/25 bg-gradient-to-br from-amber-500/10 via-transparent to-orange-500/5 p-4 sm:p-5">
        <h4 className="flex items-center gap-2 text-sm font-bold">
          <CurrencyCircleDollar className="size-4.5 text-amber-500" weight="fill" />
          Resumo Financeiro
        </h4>
  ...
  return (
    <>
      ...
      <CoverImagePicker
        ...
        hint="Adicione uma imagem quadrada para sua competição (opcional)"
      />
        ...
        {affiliateLinksEnabled && (
          <div className="animate-in fade-in-0 slide-in-from-top-1 flex flex-col gap-3 border-t border-indigo-500/20 pt-3.5 duration-200">
            <p className="text-muted-foreground flex items-start gap-2 text-xs leading-relaxed">
              <Info className="mt-0.5 size-3.5 shrink-0 text-indigo-500 dark:text-indigo-400" weight="fill" />
      ...
      <div className="border-brand-cyan/25 bg-brand-cyan/5 not-dark:border-primary/25 not-dark:bg-primary/5 rounded-2xl border p-4 sm:p-5">
        <h4 className="flex items-center gap-2 text-sm font-bold">
          <Trophy className="text-brand-mint not-dark:text-primary size-4.5" weight="fill" />
        ...
        <dl className="mt-3 flex flex-col gap-2.5 text-xs sm:text-sm">
          ...
          <SummaryRow label="Tipo">
            ...
            >
              ...
              {formData.isProOnly && (
                <Badge className="ml-1 gap-1 rounded-full border-violet-500/30 bg-violet-500/15 text-[10px] text-violet-600 dark:text-violet-400" variant="outline">
    ...
    >
      ...
      <span className="text-center">
        <span
          className={cn(
            "block text-sm font-bold transition-colors sm:text-[15px]",
            selected ? palette.text : "text-muted-foreground group-hover:text-foreground",
          )}
        >
          {title}
        </span>
